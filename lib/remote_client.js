...
class Client {
  ...
  async request(options) {
    ...
    const endpoints = this.app.config.loggerRemote.endpoint.split(',');
    const nextEndpointIndex = Math.floor((Math.random() * 100)) % endpoints.length;
    ...
    return await this.app.curl(endpoints[nextEndpointIndex], {
      content: body,
      headers,
      timeout: 30000,
      method: 'POST',
